...
function calcular() {
  ...
  if (resultado < 0) {
   ...
   resultDiv.innerHTML = "Resultado: " + resultado + " kg CO2e al día. <br>" + mensaje;
...
// Esta función muestra el resultado almacenado en localStorage
function mostrarResultado() {
  const resultadoGuardado = localStorage.getItem("resultado");
  const mensajeGuardado = localStorage.getItem("mensaje");

  if (resultadoGuardado !== null && mensajeGuardado !== null) {
    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = "Resultado: " + resultadoGuardado + " kg CO2e al día. <br>" + mensajeGuardado;
  }
}
...
function iniciar() {
  ...
  // Llama a la función calcular() para obtener el resultado inicial
  calcular();

  // Mostrar resultado si ya se ha calculado previamente
  mostrarResultado();
}
 // Mostrar el resultado
 document.getElementById("calculator-form").classList.add("show-result");
